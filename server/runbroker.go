...
func main() {
	...
	var logpath, pubport, subport string = "", "", ""
	if len(argsWithoutProg) != 4 && len(argsWithoutProg) != 6 {
		printUsage()
		return
	}
	for index, arg := range argsWithoutProg {
		if arg == "-logpath" {
			logpath = argsWithoutProg[index + 1]
			fmt.Println("logpath is: ", logpath)
		} else if arg == "-pubport" {
			pubport = argsWithoutProg[index + 1]
			fmt.Println("pubport is: ", pubport)
		} else if arg == "-subport" {
			subport = argsWithoutProg[index + 1]
			fmt.Println("subport is: ", subport)
		}
	}
